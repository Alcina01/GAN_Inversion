...
class Trainer(object):
    ...
    def train(self):
        ...
        for i, (image, category, img_path,mask,mask_path,gt,gt_path,maskpair) in enumerate(self.train_loader):
            ...
            image = image.cuda()
            mask =mask.cuda()
            maskpair =maskpair.cuda()
            category = category.cuda()
            img_path = img_path[0]

            self.model.reset_G()
            self.model.set_target(image, category, img_path,mask,maskpair,gt,i)
            ...
            loss_dict = self.model.run(i,save_interval=self.config['save_interval'])
